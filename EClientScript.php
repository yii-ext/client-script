...
class EClientScript extends CClientScript
{
    ...
    protected function combineCssFiles()
    {
        ...
        if ($this->enableCache == true) {
            $cacheName = md5(serialize($this->cssFiles));
            $cacheData = Yii::app()->cache->get($cacheName);
            if ($cacheData !== false) {
                $this->cssFiles = $cacheData;
                return;
            }
        }
        ...
        foreach ($toBeCombined as $media => $files) {
            ...
            $mediaArray = array_map('trim', explode(',', $media));
            foreach ($mediaArray as $mediaSingle) {
                if (!in_array($mediaSingle, $this->mediaTypes)) {
                    $media = '';
                }
            }
            $cssFiles[$url] = $media;
        ...
        if ($this->enableCache) {
            Yii::app()->cache->set($cacheName, $this->cssFiles);
        }
    }
     ...
     *
     * @param \CClientScript|int $type CClientScript the type of script files currently combined
     */
     ...
     * @param $code
     *
     * @internal param string $data input css data
     *
     ...
     * @param $code
     *
     * @internal param string $data script code
     *
